...
# make sure the puppet agent is started again whatever happens below
trap 'systemctl start puppet' EXIT
...
# cleanup cron jobs
cd /etc/cron.d
rm -f sticky-world-writebale.cron system-file-permissions.cron unowned-files.cron world-writebale-files.cron

# cleanup cis director
if cd /usr/share/cis_security_hardening/bin; then
  rm -f check_dot_files_write.sh check_home_dir_owner.sh check_netrc_files.sh check_passwd_group_exist.sh \
